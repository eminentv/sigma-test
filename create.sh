...
while read -r line;
do
  sigmac -t elastalert -c configs/ecs-aws-cloudtrail.yml ${AWS}/$line > update/$title/$title2/$line;
...
mkdir update/$title/$title2/logstash
...
do
  ...
  sigmac -t elastalert -c ecs-logstash ${AZURE}/$line > update/$title/$title2/logstash/$line && \
  ...
  echo "import: BaseRule.config" >> update/$title/$title2/logstash/$line && \
...
mkdir update/$title/$title2
mkdir update/$title/$title2/filebeat
mkdir update/$title/$titl e2/logstash
mkdir update/$title/$title2/winlogbeat
ls ${GCP} >> txt-rules/$title2-rules.txt
while read -r line;
do
  sigmac -t elastalert -c ecs-filebeat ${GCP}/$line > update/$title/$title2/filebeat/$line && \
  sigmac -t elastalert -c ecs-logstash ${GCP}/$line > update/$title/$title2/logstash/$line && \
  echo "import: BaseRule.config" >> update/$title/$title2/filebeat/$line && \
  echo "import: BaseRule.config" >> update/$title/$title2/logstash/$line;
done < txt-rules/$title2-rules.txt
...
do
  ...
  sigmac -t elastalert -c ecs-logstash ${o365}/$line > update/$title/$title2/logstash/$line && \
...
do
  ...
  sigmac -t elastalert -c ecs-logstash ${title}/$line > update/$title/logstash/$line && \
  sigmac -t elastalert -c winlogbeat ${title}/$line > update/$title/winlogbeat/$line && \
  echo "import: BaseRule.config" >> update/$title/filebeat/$line && \
  echo "import: BaseRule.config" >> update/$title/logstash/$line && \
  echo "import: BaseRule.config" >> update/$title/winlogbeat/$line;
done < txt-rules/$title-rules.txt
...
do
  ...
  sigmac -t elastalert -c ecs-logstash ${GENERIC}/$line > update/$title/logstash/$line && \
...
do
  ...
  sigmac -t elastalert -c ecs-logstash ${LINUX}/$line > update/$title/logstash/$line && \
  echo "import: BaseRule.config" >> update/$title/filebeat/$line && \
  echo "import: BaseRule.config" >> update/$title/logstash/$line;
done < txt-rules/$title-rules.txt
...
do
  ...
  sigmac -t elastalert -c ecs-logstash ${LINUX_AUDITD}/$line > update/$title/logstash/$line && \
  echo "import: BaseRule.config" >> update/$title/filebeat/$line && \
  echo "import: BaseRule.config" >> update/$title/logstash/$line;
done < txt-rules/$title-rules.txt
...
do
  ...
  sigmac -t elastalert -c ecs-logstash ${LINUX_MODSEC}/$line > update/$title/logstash/$line && \
  echo "import: BaseRule.config" >> update/$title/filebeat/$line && \
  echo "import: BaseRule.config" >> update/$title/logstash/$line;
done < txt-rules/$title-rules.txt
...
do
  ...
  sigmac -t elastalert -c ecs-logstash ${NETWORK}/$line > update/$title/logstash/$line && \
  echo "import: BaseRule.config" >> update/$title/filebeat/$line && \
  echo "import: BaseRule.config" >> update/$title/logstash/$line;
done < txt-rules/$title-rules.txt
...
do
  ...
  sigmac -t elastalert -c ecs-logstash ${PROXY}/$line > update/$title/logstash/$line && \
  echo "import: BaseRule.config" >> update/$title/filebeat/$line && \
  echo "import: BaseRule.config" >> update/$title/logstash/$line;
done < txt-rules/$title-rules.txt
...
do
  ...
  sigmac -t elastalert -c ecs-logstash ${WEB}/$line > update/$title/logstash/$line && \
  echo "import: BaseRule.config" >> update/$title/filebeat/$line && \
  echo "import: BaseRule.config" >> update/$title/logstash/$line;
done < txt-rules/$title-rules.txt
...
do
  ...
  sigmac -t elastalert -c ecs-logstash ${WINDOWS}/$line > update/$title/$title2/logstash/$line && \
  echo "import: BaseRule.config" >> update/$title/$title2/filebeat/$line && \
  echo "import: BaseRule.config" >> update/$title/$title2/logstash/$line && \
  echo "import: BaseRule.config" >> update/$title/$title2/winlogbeat/$line;
done < txt-rules/$title2-rules.txt
